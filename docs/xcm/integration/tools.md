...
We've prepared an xcm-tools crate which provides an easy way for users to calculate a sovereign account or XC20 asset Id. This is intended for developers or integrators, and requires basic technical knowledge to use.
...
For calculating a parachain's sovereign account address on the Relay Chain, use the following command:
...
For calculating sibling parachain's sovereign account address, use the following command:
```bash
...
You can input a standard asset Id (as seen by pallet-assets) and it will output the H160 address of that asset.

## Account32Hash

For calculating `Account32Hash` ([here](https://github.com/paritytech/polkadot/blob/master/xcm/xcm-builder/src/location_conversion.rs#L25)), we have provide a dedicated command. However, the possible `MultiLocation`s format is limited to:
