...
Source code for both the Swanky CLI and Swanky Node are hosted on GitHub:
...
Swanky CLI is a Node.js CLI app that uses Polkadot.js API on the backend, alongside many existing tools, like the cargo contract CLI. In the future, there will be many additional features to support developers, such as Wasm dApp bootstrapping via smart contract and UI scaffolding, integration tests, management of local nodes, account management, connecting and deploying contracts to both local and remote networks, compiling for various languages from a single CLI app, compatibility check from the contract pallet to the compiler, and much more.
...
This is the recommended method - which comes with all the dependencies (even Node.js), and will support auto-updates in the future.
...
2. Extract the archive to the appropriate location, for example `software` directory.
...
This approach may seem simpler, but due to the nature of node.js dependency management, may lead to compatibility or other errors. 
...
After responding to the prompts, Swanky will modify the templates, download the node, and run appropriate installation scripts.
...
:::note
Currently, you will need to be be in a project folder to run this command.
:::
...
- `grandpa` & `aura` consensus have been removed; `instant-seal` & `manual-seal` are used, instead.
...
Swanky Node is optimized for local development, while removing unnecessary components such as P2P, and more features and pallets to interact with (Contract <-> Runtime) will be added, in the future.
...
The easiest method of installation is downloading a precompiled binary from [Release Page](https://github.com/AstarNetwork/swanky-node/releases)
...
Once Rust is installed and configured, build node with:
...
If you would like to run the node in non-persistent mode, use tmp option.
...
To have only errors and contract debug output show up on the console, supply `-lerror,runtime::contracts=debug` when starting the node.
...
Once the Swanky Node is running locally, you will be able to connect to it with **Polkadot-JS Apps** front-end
...
by appending your own. A few useful commands are below:
...
Unlike other blockchains, Swanky Node adopts block authoring and finalizing gadgets called Manual Seal and Instant Seal, consensus mechanisims which are suitable for contract development and testing.

Manual seal - Blocks are authored whenever RPC called.
Instant seal - Block are authored as soon as transactions enter the pool, most often one transaction per block.
...
In addition to finalizing blocks at the time of creating them, they may also finalized later by using another RPC call, `engine_finalizeBlock`.
