...
```
- **destination** - Multilocation of destination chain
- **paymentAssetId** - Address of the local asset derivate used to pay for execution in the destination chain
- **paymentAmount** - Amount of payment asset to use for execution payment. It should cover cost of XCM instructions and Transact call weight.
...
Let's suppose the `beneficiary` is the is the EVM address `0xd43593c715fdd31c61141abd04a99fd6822c8558` of the contract. The Multilocation is `{ parents: 0, interior: [AccountKey20: { id: *EVM adddress* , network: any }] }`.   
